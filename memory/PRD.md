...
- **2026-02-14** — Added **Email template library** (`/emails`) + 8 production-ready email templates. Shared `EmailShell.jsx` provides logo header, HIPAA badge, BAA block (opt-in), footer with legal + **discreet unsubscribe + manage-preferences links** on every template. Templates: (1) Welcome + BAA — post-signup with tier-aware feature list, (2) Payment receipt — monthly subscription with big amount block + PDF invoice CTA, (3) Payment failed — "oops" with bank-reason card + grace-period timeline, (4) The Route newsletter — product updates with 3 news cards + field story, (5) Password reset — request details + didn't-request-this alert, (6) Nurse invite — agency onboarding with day-one perks, (7) License expiry — countdown card + escalation timeline, (8) HIPAA weekly digest — for directors with score, KPIs, and anomaly list. Preview harness with sidebar switcher, subject line + from address display, copy-subject button, and "No PHI · Safe" badge. Discreet dot in Landing footer links to `/emails`. All 8 templates render + include unsubscribe verified via smoke test.

- **2026-02-27** — Added **Duplicate prior note** enhancement in SOAP Editor. When starting a new SOAP note, if the selected client has a prior signed note, a "Duplicate prior note" card appears offering one-click carry-forward of the last signed S/O/A/P sections, applied template, and ICD-10 codes. Once activated, an amber "Carried forward — please review" banner replaces the card, plus a "Start blank" reset button. The `carriedFromId` field is persisted on the resulting draft/signed note for audit provenance, and the SOAPHub history now shows a terracotta "Carried forward" badge on any note derived from a prior note. Verified end-to-end via scripted smoke test (block presence, section population, banner display, clear/reset, badge visibility). Frontend-only, no backend.
